...
// breaking the other.
//
// Debug logging: set DELEGATION_GATE_DEBUG=1 in environment to enable, or
...
function debugFlagFile() {
  return process.env.DELEGATION_GATE_DEBUG_FILE || join(PLUGIN_DIR, ".debug");
}
...
function isDebugEnabled() {
  if (process.env.DELEGATION_GATE_DEBUG) return true;
...
function loadTemplates(config) {
  ...
  const defaultTemplates = {
    ...
    evolve: "Load the kd-system skill. Read the INTENT KD at {intent_kd}. Evolve the process per the scope above. Write the PROCESS KD and manage issues through the knowledge-gate tools (`issue_write`, `issue_move`, `issue_update`). Produce a PROCESS KD at {result_kd}.",
...
function extractFromText(text, fields, override = false) {
  ...
  for (const line of text.split("\n")) {
    ...
    // Match full multi-word keys (e.g. SESSION ID) as well as the
