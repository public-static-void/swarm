...
`;
  ...
  describe("PF-004: dual-registry detection — fail closed on multiple registries (AC004, AC008, AC009)", () => {
    it("returns null and emits DUAL_REGISTRY_WARNING when 2+ registries match the same session/generation (AC008)", async () => {
      const s = sid("dual-reg-1");
      await initOverseer(s);
      hooks.sessionPhaseMap.set(s, hooks.STATES.SWARM);
      hooks.sessionPhaseMap.set(`${s}:sid`, s);
      // Two milestone registries with different prefixes but the same session ID
      // — the exact PF-004 failure scenario where Pathfinder creates a duplicate.
      createKD(`milestones-feature-${s}.md`, registryContent([["M1", "pending"]]));
      createKD(`milestones-project-${s}.md`, registryContent([["M1", "pending"]]));
      ...
      try {
        ...
        expect(stderr).toContain("DUAL_REGISTRY_WARNING");
        expect(stderr).toContain("2 milestone registries");
        expect(stderr).toContain("milestones-feature");
        expect(stderr).toContain("milestones-project");
      } finally {
        process.stderr.write = origWrite;
      }
    });

    it("returns null with DUAL_REGISTRY_WARNING when 3+ registries exist", async () => {
      const s = sid("dual-reg-3");
      await initOverseer(s);
      hooks.sessionPhaseMap.set(s, hooks.STATES.SWARM);
      hooks.sessionPhaseMap.set(`${s}:sid`, s);
      createKD(`milestones-alpha-${s}.md`, registryContent([["M1", "pending"]]));
      createKD(`milestones-beta-${s}.md`, registryContent([["M2", "pending"]]));
      createKD(`milestones-gamma-${s}.md`, registryContent([["M3", "pending"]]));
      ...
      try {
        const registry = hooks.readMilestoneRegistry(s, hooks.sessionPhaseMap);
        expect(registry).toBeNull();
        expect(stderr).toContain("3 milestone registries");
        expect(stderr).toContain("milestones-alpha");
        expect(stderr).toContain("milestones-beta");
        expect(stderr).toContain("milestones-gamma");
      } finally {
        process.stderr.write = origWrite;
      }
    });

    it("returns the single registry when exactly 1 file matches (AC004 backward compat)", async () => {
      const s = sid("dual-reg-single");
      await initOverseer(s);
      hooks.sessionPhaseMap.set(s, hooks.STATES.SWARM);
      hooks.sessionPhaseMap.set(`${s}:sid`, s);
      createRegistry(s, [["M1", "in-progress"]]);
      ...
      try {
        ...
        expect(registry.rows).toEqual([{ id: "M1", state: "in-progress" }]);
        expect(stderr).not.toContain("DUAL_REGISTRY_WARNING");
      } finally {
        process.stderr.write = origWrite;
      }
    });

    it("returns null when no registries exist (0 registries preserved behavior)", async () => {
      const s = sid("dual-reg-none");
      await initOverseer(s);
      hooks.sessionPhaseMap.set(s, hooks.STATES.SWARM);
      hooks.sessionPhaseMap.set(`${s}:sid`, s);
    ...
    it("stale prior-generation registries are ignored — no false dual-registry", async () => {
      const s = sid("dual-reg-gen");
      await initOverseer(s);
      hooks.sessionPhaseMap.set(s, hooks.STATES.SWARM);
      hooks.sessionPhaseMap.set(`${s}:sid`, s);
      hooks.sessionPhaseMap.set(`${s}:gen`, 2);
      // Only one gen2 registry — the gen1 one must be filtered out
      createKD(`milestones-old-${s}-gen1.md`, registryContent([["M1", "pending"]]));
      createKD(`milestones-new-${s}-gen2.md`, registryContent([["M2", "pending"]]));
      ...
      try {
        const registry = hooks.readMilestoneRegistry(s, hooks.sessionPhaseMap);
        expect(registry).not.toBeNull();
        expect(registry.rows).toEqual([{ id: "M2", state: "pending" }]);
        expect(stderr).not.toContain("DUAL_REGISTRY_WARNING");
      } finally {
        process.stderr.write = origWrite;
      }
