...
When the repo has JS test infra (vitest config / eslint config), the suite runs via `npx vitest run` from the repository root — the canonical invocation. package.json ships with an `audit` script only; the vitest config pins collection to `tests/**` and excludes the vendored `references/` tree, so a root-level run collects exactly the swarm suite and reports its true signal. When the repo has no JS test infra, the repo's own gates apply.

## The Git Contract

git tracks swarm config: `AGENTS.md`, `agents/`, `skills/`, `plugins/`, `tests/`, `commands/`, `opencode.json`. `knowledge/` is workflow meta and stays gitignored. Verification is tree-level — working tree and tracked diffs — using the standard git workflow with hooks enabled. When the repo has JS test infra (vitest config / eslint config), a version-controlled pre-commit hook (`.githooks/pre-commit`, wired via `core.hooksPath` set by the `prepare` script on `npm install`) runs the repo gates — `npx vitest run` + `npx eslint -c eslint.security.config.mjs` — automatically at commit time and fails the commit when a gate fails. When the repo has no JS test infra, the repo's own gates apply. The Committer stays git-only; manual gate runs are the Artisan's job.
