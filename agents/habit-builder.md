...
5. **Track Issues** — Create issues with the `issue_write` tool when process friction warrants tracking. Pass `scope` (`project` or `swarm`) copied from the source KD frontmatter; the tool validates the schema, auto-assigns the per-store numeric ID, and writes `{store}/knowledge/issues/issue-{N}.md`. In Source KD References, cite durable substitutes (reports, memory entries, git commits) or quote gate-log evidence inline; runtime KDs are lifecycle-end cleanup targets and lack cross-lifecycle durability.
