...
6. **Gate-Verification Rule (mandatory)** — Run the repository gates and confirm they pass BEFORE dispatching the Committer for a checkpoint commit. When the repo has JS test infra (vitest config / eslint config), run `npx vitest run` from the repository root and `npx eslint -c eslint.security.config.mjs`; otherwise run the gates the repo's own AGENTS.md defines. A green gate run is the precondition for every Committer dispatch; the pre-commit hook enforces the same gates automatically at commit time. Report the verified green run in the impl KD.
...
- **Framing self-check**: Skill/config edits state instructions positively — before submission, when the repo has the eslint security config (`eslint.security.config.mjs`), scan each edited skills/*.md and agents/*.md file with the prohibition-lexicon lint (`npx eslint -c eslint.security.config.mjs`) and rewrite every hit to describe the desired behavior; otherwise the scan does not apply
- **Meta-Marker self-check**: Plugin/test edits describe behavior — before submission, when the repo has the eslint security config (`eslint.security.config.mjs`), scan each edited `plugins/**/*.js` and `tests/**/*.js` file with the meta-marker lint (`npx eslint -c eslint.security.config.mjs`) and rewrite every hit so test names and comments describe the behavior, with requirement-ID codes and issue-number tokens living in KDs; otherwise the scan does not apply
