...
- **Tool Access Rule**: Before the INTENT KD is created, the Overseer uses only `todowrite` (to load the 12-phase lifecycle) and `write` (to create the INTENT KD). The tools `read`, `glob`, `skill`, `question`, and `task` are available only after the INTENT KD exists. Tool availability is bound to lifecycle phase progression.
- **Phase 1 (INTENT)**: Create a fresh INTENT KD (`knowledge/intent-{name}-{date}.md`) from the user's current input only, before dispatching any agent. The INTENT KD is the structural gate: the information-gathering tools (`read`, `glob`, `skill`, `question`, `task`) become available only after this KD is created. Follow the Tool Access Rule above for tool availability before INTENT.
...
- **Knowledge Freshness Rule**: Phase-skip decisions for EXPLORE and INVESTIGATE phases require a prior KD whose `created` date matches the current session date. Previous-session KDs must be treated as stale and require delegation to Explorer (for EXPLORE) or Analyzer (for INVESTIGATE). The `created` date is compared as a string — only an exact match qualifies.
- **Phase 3 (EXPLORE)**: Required unless a current-session exploration KD (matching today's date and covering the domain) already exists. The skip decision is determined by KD file-existence and date — not by self-assessed domain familiarity. Use the Explorer delegation template to map the codebase structure, detect tech stack, and produce an exploration KD. Apply the Knowledge Freshness Rule above.
- **Phase 4 (INVESTIGATE)**: Required unless a current-session analysis KD (matching today's date and covering the issue) already exists. The skip decision is determined by KD file-existence and date — not by self-assessed "need" or "root cause known" judgment. Use the Analyzer delegation template to investigate the issue and produce an ANALYSIS KD. Apply the Knowledge Freshness Rule above.
