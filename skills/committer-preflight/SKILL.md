...
4. **Establish working branch** — Create a feature branch from the detected base branch. The Committer derives the feature branch name from the INTENT KD context using the Branch Naming Convention below. Branch establishment runs once per PREFLIGHT — CHECKPOINT and CLEANUP modes operate on the established branch.
   ...
   3. **Check for branch collision** — Compare current branch against the derived target BEFORE the existence check. Run `git branch --show-current` and compare against `<feature-branch>`.
      - **When current equals target (same-branch idempotency)**: verify workspace state (`git status --porcelain` for clean/dirty, `git stash list` for pending stash). Treat the branch as ready. Restore stashed changes if present via the adopt-then-pop flow (`git stash pop` after adopting the branch; warn and continue on pop failure per step 4.4). Write a `preflight_verdict: PASS` PREFLIGHT KD and exit. Escalation is prohibited on this path.
      - **When current differs from target**: Run `git branch --list <feature-branch>` to check whether a branch with the derived name already exists.
        - **When the branch does NOT exist**: Run `git checkout -b <feature-branch> <base>` to create and switch to the new branch.
        - **When the branch already exists (stale-branch triage)**:
          1. Gather context on the existing branch: run `git log --oneline -3 <feature-branch>` to show its recent commits, and `git rev-list --left-right --count <base>...<feature-branch>` to determine ahead/behind counts versus the base branch.
          ...
          3. **Wait for direction.** Force-update, auto-rename, silent adoption, and proceeding without approval are forbidden. Preserve any pending stash state — keep the stash intact while the collision is unresolved. Write a `preflight_verdict: ESCALATION` PREFLIGHT KD documenting the collision.
   4. **Restore stashed changes** — If step 1 stashed pending changes (dirty repo), run `git stash pop` after the working branch is established (adopt-then-pop: adopt the branch first, then pop). Preserve stash state through adopt-and-pop: keep the stash intact while any collision is unresolved, and pop after the branch is adopted. If pop fails, log a warning and continue on the adopted branch.
...
2. **Write PREFLIGHT KD** — Write a PREFLIGHT KD at the `RESULT KD` path specified in the dispatch context using the `template-preflight.md` template from the kd-system skill. The KD documents workspace setup results (branch, gitignore, etc.) and signals to the protocol-gate that PREFLIGHT is complete and can advance to EXPLORE. Every PREFLIGHT KD carries a machine-readable `preflight_verdict` frontmatter field with exactly one of the two values `PASS` (workspace ready, same-branch adopted or branch created) or `ESCALATION` (branch collision awaiting direction), plus a `## Verification` branch-ready checklist (branch name from `git branch --show-current`, clean/dirty state from `git status`). The Committer sets the field at this Exit step.
