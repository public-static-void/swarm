...
- [ ] **Agent Assignment — SWARM Milestones**: Verify every SWARM milestone assigns the phase agent (artisan) as owner. Non-artisan agent assignments in SWARM milestones are errors — flag them immediately. The Dispatch Unit field for each SWARM milestone must read: "one Artisan dispatch completes this milestone independently." Any deviation indicates a permission deadlock risk.
